...
% For instance, you can plot the magnitude of the spectrum
plot(abs(X));
...
thresholds = 3.64 * (frequencies / 1000).^(-0.8) - 6.5 * exp(-0.6 * (frequencies / 1000 - 3.3).^2) + (10^-3) * (frequencies / 1000).^4;
% Plot the threshold curve
figure;
semilogx(frequencies, thresholds); % Updated to plot 'thresholds'
xlabel('Frequency (Hz)');
...
% Copy Lk for masking
maskedLk = Lk; % Make a copy to preserve original Lk

% Compare Lk with the thresholds and mask out values where Lk < thresholds
for idx = 1:length(Lk)
    if Lk(idx) < thresholds(idx)
